...
app.get('/api/export/csv', (req, res) => {
  const filename = `PTM_Polling_Results_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  // Header row
  const headers = ['Question #', 'Question Text', 'Option #', 'Option Text'];
...
io.on('connection', (socket) => {
  ...
  // Admin batch saves questions from Question Builder
  ...
  socket.on('admin:add_question', ({ text, options }) => {
  ...
  socket.on('admin:update_question', ({ id, text, options }) => {
